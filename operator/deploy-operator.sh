...
image=docker.artifactory.braintribe.com/tribefire-cloud/tribefire-operator
tag=latest
...
function create_secrets() {
  ...
  if [[ $? != "0" ]]; then
    ...
    if [[ dry_run -eq 0 ]]; then
      err=$(kubectl create -n ${namespace} secret docker-registry bt-artifactory --docker-server=docker.artifactory.braintribe.com \
...
EOF
    if [[ ${rc} -eq 0 ]]; then
      ...
      kubectl delete etcd -n ${namespace} tf-etcd-cluster
    fi

    echo "Deploying etcd cluster in namespace ${namespace}..."
    echo "${cluster_manifest}" | sed "s|@@NAMESPACE@@|${namespace}|" |
      sed "s|@@ETCD_CLUSTER_SIZE@@|${etcd_cluster_size}|" | kubectl create -f -
